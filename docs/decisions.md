...
## ADR-028 — Bundle-level deliverables manifest as the acceptance gate (Day 20)

- **Date:** 2026-09-23 · **Status:** Accepted
- **Context:** By Day 19 the per-day test modules covered each pipeline stage and the dashboard, but no single suite asserted the *whole* committed artifact bundle that "Install→results reproducible" (Day 21 hook) and AGENTS.md §6 "dashboard data presence" depend on. Risks: a day accidentally stops writing one of its outputs (silently breaking reproducibility), a gate `gates.csv` record drifts from the in-code assertion, a `label` column regresses into causal language, or a report claims a channel "causes" conversion/revenue while every numeric table stays honest.
- **Decision:**
  1. **New `tests/test_artifacts.py`** — the manifest contract: every roadmapped artifact under `results/{tables,figures,roi,counterfactuals,sensitivity,uplift,target_segments}` and all 12 `reports/*.md` must exist and be non-empty. Section paths resolve through the config `results:` block — no hardcoded paths in tests (AGENTS.md §6).
  2. **Schema + CI contracts on the load-bearing tables:** `master_estimates` (point/se/ci/n/label), `roi_summary` (causal CI brackets the point), `evalue` (CI columns), `scenario_summary` — where the CIs are on the **estimated (Day-12 DR) scale and must sit strictly above the truth-scale net**, encoding the honest 10–38× overstatement as a visible, tested artifact property rather than prose (ADR-025).
  3. **Gate records are first-class:** every artifact `gates.csv` must have measured values and report `passed == True` — deterministic records, not just in-code asserts.
  4. **Honest vocabulary at bundle level:** every `label` column across result CSVs must contain an honest root (estimated/simulated/placebo/counterfactual/naive/observed/measured) and never "established causal/caused by/…causes"; a channel-`causes` claim in any report fails the gate (AGENTS.md §8 extends to prose).
  5. **Dashboard data contract is static-tested:** every `_read`/`_maybe` (section, name) pair in `src/dashboard/builders.py` must be in the manifest and on disk — a drift in the Day-19 reader/artifact link fails collection.
- **Consequences:**
